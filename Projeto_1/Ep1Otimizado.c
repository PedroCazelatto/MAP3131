...
/**    Exercício-Programa 01 V2 - MAP3121                     **/
...
int AlgQRcomDesl(double autovalores[max], double autovetores[max][max], double diagInf[max-1], double diagPri[max], double diagSup[max-1], double precisao, int tamanho) {
    int iteracoes, m, i, j;
    double d, mi, tau, diagSupExtra, Vextra;
    double coss[max], seno[max];
    double aux[max][max], Q[max][max], R[max][max];
...
int main() {
    ...
    double autovalores[max], ks[max], diagInf[max], diagPri[max], diagSup[max];
    ...
    if (teste == 'a') {
        ...
        if (desloc == 's') {
            ...
            iter = AlgQRcomDesl(autovalores, autovetores, diagInf, diagPri, diagSup, precisao, tamanho);
        }
        time(&end);
